...
function getPost() {
  ...
  _http.http.get('http://localhost:3000/posts').then(function (result) {
    return _ui.UI.showposts(result);
  }).catch(function (err) {
...
function submitPost() {
  ...
  var body = document.getElementById('post_field').value;
  ...
  if (title === '' || body === '') {
    _ui.UI.addAlert("Please Fill all fields", "warning");
  } else {

    if (id === "") {
      _http.http.post('http://localhost:3000/posts', data).then(function (data) {
        _ui.UI.clearFields();
        _ui.UI.addAlert('Added Successfully', 'success');
        getPost();
      }).catch(function (err) {
        return console.log(err);
      });
      ...
      _http.http.put('http://localhost:3000/posts/' + id, data).then(function (data) {
        _ui.UI.addAlert('Updated Successfully', 'success');
        _ui.UI.changeTheState('finish_update');
        getPost();
      }).catch(function (err) {
        return console.log(err);
      });
  ...
  if (e.target.id === 'delete') {
    ...
    if (confirm("Delete -> Are You Sure ")) {
      _http.http.delete('http://localhost:3000/posts/' + id).then(function (data) {
        _ui.UI.addAlert('Removed Successfully', 'warning');
        getPost();
      });
    }
  }
...
var ui = function () {
  ...
  _createClass(ui, [{
    ...
    key: 'addAlert',
    value: function addAlert(text, className) {
      var _this = this;
      ...
      var post = document.querySelector('.posts-container');
      body.insertBefore(div, post);
      this.submit_btn.style.display = "none";
      setTimeout(function () {
        _this.removeAlert();
        _this.submit_btn.style.display = "block";
      }, 2000);
    }
  }, {
    key: 'removeAlert',
    ...
    key: 'fillAllForms',
    value: function fillAllForms(data) {
    ...
    key: 'changeTheState',
    value: function changeTheState(type) {
      ...
      if (type === "update") {
        ...
        var cancelinsert = document.querySelector('cancel-before');
